...
def LabelColor(img, gt, pre, seg):
    ...
    disTable = np.zeros((256, NUM_OF_CLASSESS, NUM_OF_CLASSESS))
    ...
    for i in range(height):
        for j in range(width):
            ...
            disTable[img[i,j,0]][gt[i,j,0]][pre[i,j,0]] += 1
            # Label color of gt
...
def OutputResult(table, disTable):
    fout = open('result_seg_distance.txt', 'w')
    fout.write('%d\n' % NUM_OF_CLASSESS)
    ...
    fout.write('--------------distance table-------------\n')
    fout.write('dis\t1\t2\t3\t4\t5\t6\t7\t8\n')
    for o in range(0,16):
        tmpTable = np.zeros((NUM_OF_CLASSESS, NUM_OF_CLASSESS))
        for delta in range(o*16+0,o*16+16):
            tmpTable += disTable[delta]
        fout.write('%d\t' % (o))
        for i in range(1,NUM_OF_CLASSESS):
            tp = fp = fn = 0
            tp = tmpTable[i,i].astype(int)
            for j in range(1, NUM_OF_CLASSESS):
                if i != j:
                    fp += tmpTable[j,i].astype(int)
                    fn += tmpTable[i,j].astype(int)
            if (tp + fp + fn != 0):
                IoU = float(tp) / float(tp + fp + fn)
            else:
                IoU = 0
            fout.write('%.6f\t' % (float(IoU)))
        fout.write('\n')
    fout.close()
...
cntDisTable = np.zeros((256, NUM_OF_CLASSESS, NUM_OF_CLASSESS))
...
videoWriter = cv2.VideoWriter('test_seg.avi', cv2.cv.CV_FOURCC('M', 'J', 'P', 'G'), 5, (IMAGE_WIDTH, IMAGE_HEIGHT * 3), True)
...
for i in range(len(imgList)):
    ...
    table, segImg, disTable = LabelColor(img, gt, pre, seg)
    cntTable += table
    cntDisTable += disTable
...
OutputResult(cntTable, cntDisTable)
