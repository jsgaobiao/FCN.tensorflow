...
tf.flags.DEFINE_string("logs_dir", "logs/model_0703_finetune/", "path to logs directory")
tf.flags.DEFINE_string("vis_dir", "logs/vis/test_finetune/", "path to save results of visualization")
...
w3M = tf.constant(0.1/float((2*R+1)*(2*R+1)-1))
...
def RandomCrop(images, annotations):
    h = random.randrange(0, IMAGE_HEIGHT - IMAGE_HEIGHT + 1, 1)
    w = random.randrange(0, IMAGE_WIDTH - IMAGE_WIDTH + 1, 1)
    return images[:,h:h + IMAGE_HEIGHT, w:w + IMAGE_WIDTH_CROP, :], annotations[:,h:h + IMAGE_HEIGHT, w:w + IMAGE_WIDTH_CROP, :]
